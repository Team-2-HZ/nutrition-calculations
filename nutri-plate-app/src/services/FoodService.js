...
export async function getFood(food, grams) {
    ...
    if(filteredFoods.length === 0){
        try{
            const response = await fetch(`https://api.edamam.com/api/food-database/v2/parser?ingr=${food}&app_id=${APP_ID}&app_key=${APP_KEY}`);
            ...
            if(data){
                ...
                const foodDetails = {
                    ...
                    foodMeasures: data.hints[0].measures.find((measure) => measure.label === 'Gram'),
                }
                ...
                const nutritionResponse = await fetch(`https://api.edamam.com/api/food-database/v2/nutrients?app_id=${APP_ID}&app_key=${APP_KEY}`, {
                    method: 'POST',
                    headers: {
